# Hi 👋, I'm Akshita Gupta  

### 🚀 DevOps Engineer (Fresher) | Cloud & Automation Enthusiast  

📍 Indore, India  
📧 akshitag382@gmail.com  
🔗 [LinkedIn](https://linkedin.com/in/akshita-gupta) | 💻 [GitHub](https://github.com/AKSHITAGUPTA11)

---

## 👩‍💻 About Me  

I’m an **enthusiastic and detail-oriented DevOps Engineer (Fresher)** with a strong foundation in **Linux, AWS Cloud, CI/CD pipelines, and automation tools**.  
I enjoy working on **cloud infrastructure, containerization, and deployment automation** to build scalable and reliable systems.

💡 I’m passionate about:
- Cloud-native technologies  
- Automation & DevOps best practices  
- Continuous learning and improvement  

---

## 🔧 Tech Stack  

### ☁️ DevOps & Cloud  
- **AWS** (EC2, S3, IAM, EBS, CloudWatch, Lambda)  
- Docker | Kubernetes | Jenkins  
- Terraform | Ansible  
- Linux  

### ⚙️ CI/CD & Version Control  
- Git | GitHub  
- GitHub Actions | Jenkins  

### 🧑‍💻 Programming & Scripting  
- Python | Bash  
- C | C++ | JavaScript  

### 📊 Monitoring & Databases  
- Prometheus (Basic)  
- MySQL | MongoDB  

### 🌐 Web Technologies  
- HTML | CSS | React.js  

---

## 📌 Projects  

### 🔹 Automated Deployment Pipeline (DevOps Project)  
- Built a **CI/CD pipeline** using Jenkins and GitHub  
- Automated build & deployment process  
- Used **Docker** for containerized deployments  
- Deployed applications on **AWS EC2**  
- Configured monitoring using **AWS CloudWatch**

### 🔹 Disaster Management App  
- Developed a **real-time disaster alert application**  
- Tech Stack: React Native & MongoDB  
- Deployed backend services on **AWS EC2**

### 🔹 WatchKart Website  
- Created a responsive **e-commerce website**  
- Built using HTML, CSS, and JavaScript  
- Used Git & GitHub for version control  

---

## 🎓 Education  

**Bachelor of Technology – Information Technology**  
📍 Chameli Devi Group of Institutions, Indore  
📅 Nov 2021 – Jul 2025  
📊 CGPA: **7.5 / 10**

---

## 📜 Certifications  

- 🏅 **AWS Cloud Foundation** – Amazon Web Services  
- 🏅 **DevOps Basics** – Coursera / Great Learning  
- 🏅 **Web Development (JavaScript & React.js)** – CodeMantra  
- 🏅 **React.js & Node.js** – Amenses Innovation  

---

## 🌱 Currently Learning  

- Advanced AWS services  
- Kubernetes & Cloud-Native architectures  
- Infrastructure as Code (IaC) best practices  

---

## 🤝 Looking to Collaborate On  

- Open-source **DevOps & Cloud projects**  
- CI/CD automation  
- Cloud infrastructure & deployment workflows  

---

## ✨ Strengths  

Quick Learner | Problem Solver | Focused  
Collaborative | Continuous Learner  

---

## 💬 Interests  

☁️ Cloud Infrastructure  
⚙️ Automation  
🌍 Open-Source Projects  
✈️ Traveling  
🤝 Social Work  

---

## 📊 GitHub Stats  

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=AKSHITAGUPTA11&show_icons=true&theme=github_dark&hide_border=true" height="180"/>
  <img src="https://streak-stats.demolab.com?user=AKSHITAGUPTA11&theme=github-dark&hide_border=true" height="180"/>
</p>

<p align="center">
  <img
    src="https://github-readme-stats.vercel.app/api/top-langs?username=AKSHITAGUPTA11&layout=compact&langs_count=6&theme=github_dark&hide_border=true&cache_seconds=1800"
    height="160"
  />
</p>




⭐ *Feel free to explore my repositories and connect with me!*  
